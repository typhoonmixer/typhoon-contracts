...
var W = (t, e, n) => (K(t, typeof e != "symbol" ? e + "" : e, n), n), Q = (t, e, n) => {
  if (!e.has(t))
    throw TypeError("Cannot " + n);
};
var E = (t, e, n) => (Q(t, e, "read from private field"), n ? n.call(t) : e.get(t)), z = (t, e, n) => {
  if (e.has(t))
...
function satisfy(t, e) {
  ...
  for (const g of o) {
    ...
    const [, y, , A, h, p, w] = M, I = {
      operator: y,
      version: combineVersion(A, h, p, w),
      major: A,
      minor: h,
      patch: p,
...
function generateSnapshotFromManifest(t) {
  ...
  var y = t.metaData, A = y.remoteEntry, h = A.path, p = A.name, w = A.type, I = y.types, O = y.buildInfo.buildVersion, L = y.globalName, T = t.exposes, S = {
    version: l || "",
    buildVersion: O,
    globalName: L,
    remoteEntry: simpleJoinRemoteEntry(h, p),
    remoteEntryType: w,
...
function generatePreloadAssets(t, e, n, r, o) {
  ...
  traverseModuleInfo(r, n, (h, p, w) => {
    let I;
    if (w)
      I = d;
    else if (Array.isArray(g)) {
      const D = g.find((b) => b.nameOrAlias === p.name || b.nameOrAlias === p.alias);
      if (!D)
        return;
    ...
    let L = "modules" in h ? h.modules : [];
    const T = normalizePreloadExposes(I.exposes);
    if (T.length && "modules" in h) {
      var S;
      L = h == null || (S = h.modules) == null ? void 0 : S.reduce((D, b) => ((T == null ? void 0 : T.indexOf(b.moduleName)) !== -1 && D.push(b), D), []);
    }
    function _(D) {
      const b = D.map((N) => getResourceUrl(h, N));
      return I.filter ? b.filter(I.filter) : b;
    }
    if (L) {
      const D = L.length;
      for (let b = 0; b < D; b++) {
        const N = L[b], P = `${p.name}/${N.moduleName}`;
        t.hooks.lifecycle.handlePreloadModule.emit({
          id: N.moduleName === "." ? p.name : P,
          name: p.name,
          remoteSnapshot: h,
          preloadConfig: I,
          remote: p,
          origin: t
        }), !getPreloaded(P) && (I.resourceCategory === "all" ? (s.push(..._(N.assets.css.async)), s.push(..._(N.assets.css.sync)), i.push(..._(N.assets.js.async)), i.push(..._(N.assets.js.sync))) : (I.resourceCategory = "sync") && (s.push(..._(N.assets.css.sync)), i.push(..._(N.assets.js.sync))), setPreloaded(P));
      }
    }
  }, !0, {}, o), o.shared && o.shared.forEach((h) => {
    var p;
    const w = (p = u.shared) == null ? void 0 : p[h.sharedName];
    if (!w)
      return;
    const I = getRegisteredShare(t.shareScopeMap, h.sharedName, w, t.hooks.lifecycle.resolveShare);
    I && typeof I.lib == "function" && (h.assets.js.sync.forEach((O) => {
      l.add(O);
...
class FederationHost {
  ...
  initializeSharing(e = DEFAULT_SCOPE, n) {
    ...
    const s = r[e], i = (u, d) => {
      ...
      s[u] = s[u] || {};
      const A = s[u], h = A[M], p = Boolean(h && (h.eager || ((g = h.shareConfig) == null ? void 0 : g.eager)));
      (!h || h.strategy !== "loaded-first" && !h.loaded && (Boolean(!y) !== !p ? y : o > h.from)) && (A[M] = d);
